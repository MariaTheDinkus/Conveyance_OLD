...
public class CatwalkBlock extends Block implements BlockWrenchable {
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView view, BlockPos pos, EntityContext context) {
        ...
        if (context instanceof EntityContextImpl) {
			Item heldItem = ((EntityContextImplAccess) context).getHeldItem();
    ...
    @Override
    public VoxelShape getCollisionShape(BlockState state, BlockView view, BlockPos pos, EntityContext entityContext) {
        Box bottom = new Box(0, 0, 0, 1, (1F / 16F), 1);
        Box north = new Box(0, 0, 0, 1, 1, (1F / 16F));
        Box east = new Box((15F / 16F), 0, 0, 1, 1, 1);
        Box south = new Box(0, 0, (15F / 16F), 1, 1, 1);
